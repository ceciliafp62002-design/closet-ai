...
/* ═══════════════════════════════════════
★ FIX 1 — JSON PARSE ROBUSTO
Resuelve: “JSON Parse error: Expected ‘]’”
Claude a veces devuelve `json ... ` o texto extra.
Esta función extrae el JSON aunque venga sucio.
═══════════════════════════════════════ */
function safeParseJSON(text) {
if (!text) throw new Error(“Respuesta vacía de la IA”);
// 1. Elimina markdown fences
let cleaned = text.replace(/`(?:json)?\s*/gi, "").replace(/`/g, “”).trim();
// 2. Intento directo
try { return JSON.parse(cleaned); } catch (_) {}
// 3. Busca primer objeto JSON {…}
const objMatch = cleaned.match(/({[\s\S]*})/);
if (objMatch) { try { return JSON.parse(objMatch[1]); } catch (*) {} }
// 4. Busca primer array JSON […]
const arrMatch = cleaned.match(/([[\s\S]*])/);
if (arrMatch) { try { return JSON.parse(arrMatch[1]); } catch (*) {} }
// 5. Intenta reparar JSON truncado
try { return JSON.parse(cleaned + “]}”); } catch (_) {}
throw new Error(`Respuesta inválida de la IA. Intenta de nuevo.`);
}

/* ═══════════════════════════════════════
★ FIX 2 — SESIÓN PERSISTENTE
Resuelve: tener que registrarse en cada apertura.
Se guarda/restaura automáticamente desde localStorage.
═══════════════════════════════════════ */
...
// Verifica que el usuario aún existe en la BD local
...
/* ═══════════════════════════════════════
★ FIX 3 — CONTADOR DE USOS API
Resuelve: saber cuántas llamadas gratuitas quedan.
Resetea automáticamente cada mes.
═══════════════════════════════════════ */
...
<p style={{ color: “#ff6b6b”, fontSize: 10, margin: 0, lineHeight: 1.5 }}>
⚠️ Se usará la foto completa hasta el próximo mes.
</p>
</div>
)}
</div>
...
Analyze the image and identify ALL visible garments (IGNORE: phone cases, technology, bags unless clearly a fashion bag, hands, people’s faces, furniture).
...
Only include garments with confidence >= 0.6. Be precise with bounding boxes.
...
body: body:JSON.stringify({model:"claude-haiku-4-5",max_tokens:10,messages:[{role:"user",content:"Hi"}]});
...
// ★ Usa safeParseJSON en lugar de JSON.parse directo
...
if (!res.ok) return null; // fallback silencioso si falla

incrementApiUsage(“photoroom”);
return await res.blob(); // PNG con transparencia
}

/* ─── COLOCAR SOBRE FONDO BLANCO ─────── */
...
img.onerror = reject;
img.src = typeof blobOrDataUrl === “string”
? blobOrDataUrl
: URL.createObjectURL(blobOrDataUrl);
});
}

/* ─── IMAGEN FONDO BLANCO (fallback sin API) ─── */
...
```
// 2. Fallback: recorte por bbox si es preciso
const bbox = { x: p.bbox_x ?? 0, y: p.bbox_y ?? 0, w: p.bbox_w ?? 1, h: p.bbox_h ?? 1 };
const isFullImage = bbox.w > 0.9 && bbox.h > 0.9;

if (prendas.length === 1 || isFullImage) {
  return garmentImageSingle(dataUrl);
}
...
/* ─── CSS ───────────────────────────── */
const CSS = `@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@600;700&family=Sora:wght@400;500;600;700&display=swap'); *,*::before,*::after{box-sizing:border-box;margin:0;padding:0} html,body,#root{height:100%;height:100dvh;overflow:hidden;background:#08080A} ::-webkit-scrollbar{display:none} :root{--sat:env(safe-area-inset-top,0px);--sab:env(safe-area-inset-bottom,0px)} input,button,select,textarea{font-family:'Sora',system-ui,sans-serif} input::placeholder,textarea::placeholder{color:#44444E} @keyframes spin  {to{transform:rotate(360deg)}} @keyframes pulse {0%,100%{opacity:1}50%{opacity:0.3}} @keyframes rise  {from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}} @keyframes risefast {from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}} @keyframes bar   {from{width:0}to{width:100%}} @keyframes glow  {0%,100%{box-shadow:0 0 18px rgba(201,240,78,0.15)}50%{box-shadow:0 0 36px rgba(201,240,78,0.4)}} @keyframes fadein{from{opacity:0}to{opacity:1}} @keyframes shake {0%,100%{transform:translateX(0)}25%{transform:translateX(-5px)}75%{transform:translateX(5px)}}`;
...
if (mode === "login") {
  ...
  DB.saveSession(u); // ★ persiste sesión
...
/* ═══════════════════════════════════════
API KEY SCREEN — incluye PhotoRoom key + contador
═══════════════════════════════════════ */
...
```
    ...
    <div style={{marginTop:22,marginBottom:10,borderTop:`1px solid ${T.border}`,paddingTop:18}}>
      ...
      <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:10,padding:"9px 12px",marginBottom:12}}>
        <p style={{color:T.muted,fontSize:11,lineHeight:1.6,margin:0}}>
          ...
          {" "}— <strong style={{color:T.dim}}>500 recortes gratis/mes</strong>
        </p>
      </div>
      ...
      <div style={{marginBottom:10}}>
        ...
        <input type="text" value={prKey} onChange={e=>sprk(e.target.value)}
          placeholder="pr_api_..."
    ...
    <div style={{marginTop:11,padding:"10px 13px",background:"rgba(201,240,78,0.05)",border:`1px solid ${T.aLow}`,borderRadius:10}}>
      <p style={{color:T.muted,fontSize:10,lineHeight:1.7,margin:0}}>
        🔒 Las keys se guardan en <strong style={{color:T.dim}}>localStorage</strong>. Nadie externo puede acceder a ellas.
      </p>
    </div>
  </div>
</div>
...
/* ═══════════════════════════════════════
SCANNER SCREEN — con PhotoRoom integrado
═══════════════════════════════════════ */
...
scd(prev => { if (prev <= 1) { clearInterval(timerRef.current); return 0; } return prev - 1; });
}, 1000);
...
try {
  ...
  // ★ Usa pipeline con PhotoRoom si hay key
...
```
    {/* ★ Mini contador PhotoRoom en scanner */}
    {photoRoomKey && (
      <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:10,padding:"8px 12px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          ...
          <span style={{color:T.dim,fontSize:11}}>Recorte con IA</span>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {(() => {
            const u = getApiUsage("photoroom");
            const color = u.remaining === 0 ? T.err : u.remaining <= 100 ? "#FF9F0A" : T.ok;
            return (
              <>
                <span style={{color,fontSize:11,fontWeight:700}}>{u.remaining}</span>
                <span style={{color:T.muted,fontSize:10}}>/ 500 restantes</span>
              </>
            );
          })()}
        </div>
...
if (saving) return (
  <div style={{height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:T.bg,gap:16,padding:26}}>
    <div style={{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center",marginBottom:8}}>
      {prendas.filter((_,i)=>selected[i]).map((_,idx)=>{
        const origIdx = prendas.indexOf(prendas.filter((_,j)=>selected[j])[idx]);
        return (
          <div key={idx} style={{width:70,height:70,borderRadius:14,overflow:"hidden",border:`2px solid ${T.accent}`,background:"#fff"}}>
            <img src={garmentImgs[origIdx]} alt="" style={{width:"100%",height:"100%",objectFit:"contain"}} />
          </div>
        );
      })}
    </div>
...
{g.imageUrl?<img src={g.imageUrl} alt=”” style={{width:“100%”,height:“100%”,objectFit:“contain”,display:“block”}} />:<div style={{width:“100%”,height:“100%”,background:g.color,display:“flex”,alignItems:“center”,justifyContent:“center”,fontSize:20}}>{g.emoji}</div>}
...
onUpdate(updated); swt(true); setTimeout(()=>swt(false),2000);
}
return (
...
{garment.imageUrl?<img src={garment.imageUrl} alt=”” style={{width:“100%”,height:“100%”,objectFit:“contain”,display:“block”}} />:<div style={{width:“100%”,height:“100%”,background:garment.color,display:“flex”,alignItems:“center”,justifyContent:“center”,fontSize:80}}>{garment.emoji}</div>}
...
/* ═══════════════════════════════════════
PROFILE SCREEN — con contador PhotoRoom
═══════════════════════════════════════ */
...
<button onClick={onConfig} title=“Configurar APIs”
...
1. restoreSession() al montar → sesión persistente
1. photoRoomKey en estado
1. ApiKeyScreen recibe onSavePhotoRoom
1. ScannerScreen recibe photoRoomKey
1. ProfileScreen muestra contador
   ═══════════════════════════════════════ */
...
// ★ FIX SESIÓN: restaura automáticamente al montar
useEffect(()=>{
...
ss(“app”); // ← va directo al armario, sin pasar por login
}
},[]);
...
```
    ...
    {/* API KEY — ahora con PhotoRoom */}
    {screen==="apikey" && (
      <ApiKeyScreen
        onSave={saveKey}
        onBack={()=>ss("app")}
        current={apiKey}
        currentPhotoRoom={photoRoomKey}
        onSavePhotoRoom={savePhotoRoomKey}
      />
    )}

    {/* SCANNER — ahora con photoRoomKey */}
    {screen==="scanner" && (
      <ScannerScreen
        onSave={saveGarment}
        onBack={()=>ss("app")}
        apiKey={apiKey}
        photoRoomKey={photoRoomKey}
        onNeedKey={()=>ss("apikey")}
      />
    )}
